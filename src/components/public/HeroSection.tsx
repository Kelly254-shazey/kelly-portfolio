...
import { ArrowDown, Mail, MessageCircle, ChevronLeft, ChevronRight } from 'lucide-react'
...
export function HeroSection({ profilePhotos = [], resumeUrl = null }: { profilePhotos?: string[]; resumeUrl?: string | null }) {
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="absolute top-1/3 right-1/3 w-48 h-48 bg-purple-500/5 rounded-full blur-3xl animate-float" />
        ...
        <div className="grid gap-12 lg:grid-cols-2 lg:gap-16 items-center">
          <div className="space-y-8">
            <motion.div
              ...
              className="space-y-4"
            >
              <div className="inline-flex items-center gap-2 rounded-full border border-primary-500/20 bg-white/40 backdrop-blur-sm px-4 py-1.5 text-sm text-primary-600">
            ...
            >
              ...
              {resumeUrl ? (
                <Button size="lg" variant="outline" icon={<ArrowDown className="h-4 w-4" />} onClick={handleReadResume}>
                  Read Resume
                </Button>
              ) : (
                <Button size="lg" variant="outline" icon={<ArrowDown className="h-4 w-4" />} disabled>
                  Read Resume
                </Button>
              )}
            </motion.div>
                    ...
                    {profilePhotos.length > 1 && (
                      <>
                        <button
                          ...
                          className="absolute left-2 top-1/2 -translate-y-1/2 rounded-full bg-slate-900/60 p-1.5 text-white hover:bg-slate-900/80 transition-colors"
                        >
                        ...
                        <button
                          ...
                          className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full bg-slate-900/60 p-1.5 text-white hover:bg-slate-900/80 transition-colors"
                        >
